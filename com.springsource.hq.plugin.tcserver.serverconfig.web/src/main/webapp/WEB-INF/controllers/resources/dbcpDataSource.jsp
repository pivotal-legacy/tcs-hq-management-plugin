...
<div id="content">
  ...
  <h1>
    Settings for <c:out value="${dbcpDataSource.general.jndiName}" />
  ...
  <spring:url value="/app/{settingsId}/resources/jdbc/dbcp/{dataSourceId}/" var="url">
    <spring:param name="settingsId" value="${settings.humanId}" />
    <spring:param name="dataSourceId" value="${dbcpDataSource.humanId}" />
  </spring:url>
  <form:form action="${url}" modelAttribute="dbcpDataSource" method="put">
    <jsp:include page="dbcpDataSource-inc.jsp" />
    <br />
    <c:if test="${!readOnly}">
	    <input type="submit" class="spring-js-submit" value="Save" />
    </c:if>
  </form:form>
  <spring:hasBindErrors name="dbcpDataSource">
