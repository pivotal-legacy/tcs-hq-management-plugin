...
/**
 * Implementation of {@link Command} responsible for adding a server to a group
 */
public class AddServerToGroupCommand implements Command, InitializingBean {

    protected static final String COMMAND_NAME = "add-server-to-group";

    protected static final String COMMAND_NAME_DESC = "Adds a SpringSource tc Server to a Compatible Group/Cluster of SpringSource tc Servers";
    ...
    /**
     * 
     * @param groupRepository The {@link GroupRepository} to use for adding a server to a group
     ...
     */
    public AddServerToGroupCommand(GroupRepository groupRepository, OptionParser optionParser, PrintWriter outWriter, PrintWriter errorWriter) {
    ...
    public int execute(String[] args) throws IOException {
        ...
        if (!options.has(OPT_GROUP_NAME) && !options.has(OPT_GROUP_ID)) {
            errorWriter.println("Either the group name or the ID of a group must be specified");
            return 1;
        }
        ...
        final GroupsResponse groupResponse = groupRepository.addServerToGroup(group, server);
