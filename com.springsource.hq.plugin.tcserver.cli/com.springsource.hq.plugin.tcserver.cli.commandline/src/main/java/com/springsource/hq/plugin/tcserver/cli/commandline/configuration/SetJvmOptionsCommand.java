...
public class SetJvmOptionsCommand implements Command, InitializingBean {

    protected static final String COMMAND_NAME = "set-jvm-options";

    protected static final String COMMAND_NAME_DESC = "Sets the JVM options of a SpringSource tc Server or group of SpringSource tc Servers";
    ...
    protected static final String OPT_OPTIONS = "options";

    protected static final String OPT_OPTIONS_DESC = "A comma-separated list of JVM options to set. The equals sign after --options is required.";
     ...
     */
    public SetJvmOptionsCommand(ConfigurationRepository configurationRepository, OptionParser optionParser, PrintWriter outWriter,
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        optionParser.accepts(OPT_OPTIONS, OPT_OPTIONS_DESC).withOptionalArg().ofType(String.class);
    }
    ...
    public int execute(String[] args) throws IOException {
        ...
        if (!options.has(OPT_OPTIONS)) {
            errorWriter.println("Missing required argument --" + OPT_OPTIONS);
            return 1;
        }
        ...
        final String jvmOptions = (String) options.valueOf(OPT_OPTIONS);
        if (options.has(OPT_SERVER_ID) || options.has(OPT_SERVER_NAME)) {
            ...
            final JvmOptionsResponse response = configurationRepository.setJvmOptions(server, parseJvmOptions(jvmOptions));
        ...
        final JvmOptionsResponse response = configurationRepository.setJvmOptions(group, parseJvmOptions(jvmOptions));
        return handleResponse(response);
    }
        ...
        if (response.getStatus() == ResponseStatus.FAILURE) {
            ...
            for (StatusResponse statusResponse : response.getStatusResponse()) {
                if (statusResponse.getStatus() == ResponseStatus.FAILURE) {
                    ...
                    errorWriter.println("Failed to set JVM options on " + statusResponse.getResourceName() + ".  Reason: "
    ...
    private JvmOptions parseJvmOptions(final String options) {
        final List<String> jvmOptions = new ArrayList<String>();
        if (options == null || "\"\"".equals(options)) {
            return new JvmOptions();
        }
        String jvmOpts = options;
        int commaindex = jvmOpts.indexOf(",");
        while (commaindex != -1) {
            if (commaindex > 0) {
                if ('\\' == jvmOpts.charAt(commaindex - 1)) {
                    commaindex = jvmOpts.indexOf(",", commaindex + 1);
                } else {
                    jvmOptions.add(jvmOpts.substring(0, commaindex).trim().replaceAll("\\\\,", ","));
                    jvmOpts = jvmOpts.substring(commaindex + 1);
                    commaindex = jvmOpts.indexOf(",");
                }
            } else {
                // String starts with comma
                jvmOpts = jvmOpts.substring(1);
                commaindex = jvmOpts.indexOf(",");
            }
        }
        if (!("".equals(jvmOpts.trim().replaceAll("\\\\,", ",")))) {
            jvmOptions.add(jvmOpts.trim().replaceAll("\\\\,", ","));
        }
        JvmOptions vmOptions = new JvmOptions();
        vmOptions.getOption().addAll(jvmOptions);
        return vmOptions;
    }
