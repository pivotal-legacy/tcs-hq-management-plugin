...
/**
 * Implementation of {@link Command} responsible for restarting a tc Server or group of tc Servers
 */
public class RestartCommand implements Command, InitializingBean {

    protected static final String COMMAND_NAME = "restart";

    protected static final String COMMAND_NAME_DESC = "Restarts a SpringSource tc Server or a group of SpringSource tc Servers";
    ...
    /**
     * 
     * @param controlOperationInvoker The {@link ControlOperationInvoker} to use for invoking the restart operation
     ...
     */
    public RestartCommand(ControlOperationInvoker controlOperationInvoker, OptionParser optionParser, PrintWriter outWriter, PrintWriter errorWriter) {
    ...
    public int execute(String[] args) throws IOException {
        ...
        if (options.has(OPT_SERVER_ID) || options.has(OPT_SERVER_NAME)) {
            ...
            final ControlStatusResponse response = controlOperationInvoker.restart(server);
        ...
        final ControlStatusResponse response = controlOperationInvoker.restart(group);
        return handleResponse(response);
    }
        ...
        if (response.getStatus() == ResponseStatus.FAILURE) {
            ...
            for (StatusResponse statusResponse : response.getStatusResponse()) {
                if (statusResponse.getStatus() == ResponseStatus.FAILURE) {
                    ...
                    errorWriter.println("Failed to restart " + statusResponse.getResourceName() + ".  Reason: "
