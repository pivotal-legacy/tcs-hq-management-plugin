...
/**
 * Implementation of {@link Command} responsible for stopping an application deployed to a tc Server or group of tc
 * Servers
 */
public class StopApplicationCommand implements Command, InitializingBean {

    protected static final String COMMAND_NAME = "stop-application";

    protected static final String COMMAND_NAME_DESC = "Stops an application deployed to a SpringSource tc Server or a group of SpringSource tc Servers";
    ...
    /**
     * 
     * @param applicationManager The {@link ApplicationManager} to use for stopping applications
     ...
     */
    public StopApplicationCommand(ApplicationManager applicationManager, OptionParser optionParser, PrintWriter outWriter, PrintWriter errorWriter) {
    ...
    public int execute(String[] args) throws IOException {
        ...
        if (options.has(OPT_SERVER_ID) || options.has(OPT_SERVER_NAME)) {
            ...
            final ApplicationManagementResponse response = applicationManager.stopApplication(server, service, host, application);
        ...
        final ApplicationManagementResponse response = applicationManager.stopApplication(group, service, host, application);
        return handleResponse(response);
    }
        ...
        if (response.getStatus() == ResponseStatus.FAILURE) {
            ...
            for (StatusResponse statusResponse : response.getStatusResponse()) {
                if (statusResponse.getStatus() == ResponseStatus.FAILURE) {
                    ...
                    errorWriter.println("Failed to stop application on " + statusResponse.getResourceName() + ".  Reason: "
