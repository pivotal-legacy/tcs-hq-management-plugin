...
/**
 * Implementation of {@link Command} responsible for starting a tc Server or group of tc Servers
 */
public class StartCommand implements Command, InitializingBean {

    protected static final String COMMAND_NAME = "start";

    protected static final String COMMAND_NAME_DESC = "Starts a SpringSource tc Server or a group of SpringSource tc Servers";
    ...
    /**
     * 
     * @param controlOperationInvoker The {@link ControlOperationInvoker} to use for invoking the start operation
     ...
     */
    public StartCommand(ControlOperationInvoker controlOperationInvoker, OptionParser optionParser, PrintWriter outWriter, PrintWriter errorWriter) {
    ...
    public int execute(String[] args) throws IOException {
        ...
        if (options.has(OPT_SERVER_ID) || options.has(OPT_SERVER_NAME)) {
            ...
            final ControlStatusResponse response = controlOperationInvoker.start(server);
        ...
        final ControlStatusResponse response = controlOperationInvoker.start(group);
        return handleResponse(response);
    }
        ...
        if (response.getStatus() == ResponseStatus.FAILURE) {
            ...
            for (StatusResponse statusResponse : response.getStatusResponse()) {
                if (statusResponse.getStatus() == ResponseStatus.FAILURE) {
                    ...
                    errorWriter.println("Failed to start " + statusResponse.getResourceName() + ".  Reason: "
